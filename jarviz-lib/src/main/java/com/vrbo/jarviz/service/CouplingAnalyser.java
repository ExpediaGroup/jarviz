...
public class CouplingAnalyser {
    ...
    private void init(final JarvizConfig jarvizConfig) {
        ...
        JarvizConfig effectiveConfig = jarvizConfig;
        if (jarvizConfig == null) {
            log.info("JarvizConfig is not found, using default config.");
            effectiveConfig = JarvizConfig.createDefaultConfig();
        }

        final String artifactDir = effectiveConfig.getArtifactDirectory();
        log.info("Artifact directory: {}", artifactDir);
        if (!getOrCreateDirectory(artifactDir).exists()) {
            throw new IllegalStateException("Failed to initiate artifact directory " + artifactDir);
        }
    }
